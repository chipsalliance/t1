...
stdenv.mkDerivation rec {
  ...
  # We need a writable source
  postUnpack = ''
    unpack() {
      local s="$1"; shift
      local d="$1"; shift

      cp -pr --reflink=auto -- "$s" "$d"
      eval ''${d}Src="$(realpath ./$d)"
    }

    unpack ${binutilsSrc} binutils
    unpack ${gccSrc} gcc
    unpack ${newlibSrc} newlib
    unpack ${gdbSrc} gdb
  ...
  preConfigure = ''
    local allSrcs=(gcc gdb binutils newlib)
    local srcVar
    # Add "--with-gcc-src=$gccSrc" into configureFlags
    for name in ''${allSrcs[@]}; do
      srcVar="$name"Src
      configureFlagsArray+=(--with-"$name"-src=''${!srcVar})
      unset "$name"Src
    done
  '';
