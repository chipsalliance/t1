...
set_config() {
  ...
  local allConfigsArray=( $(list_config) )

  local verifiedConfig=""
  for c in ${allConfigsArray[@]}; do
    if [[ "$c" == "$config" ]]; then
      verifiedConfig="$c"
    fi
  done
  if [[ -z "$verifiedConfig" ]]; then
    error "Config '$config' is not supported, available configs: ${allConfigsArray[*]}"
    return 1
  fi
...
build() {
  ...
  info "Building $fullAttr to result dir ./result-$attr with flag '$@'"
  nix build --no-warn-dirty --print-build-logs --out-link result-$attr "$fullAttr" $@
}

test() {
  if [[ -z "$@" || "$@" = "help" ]]; then
    error "Use 'test <case-name> [emulator]' to run a test"
    return 1
  fi
  ...
  local hasAttr=$(nix eval --json --no-warn-dirty ".#t1.$_CFG.ip.cases" --apply "cases: cases ? $case")
  ...
  nix build --no-warn-dirty --print-build-logs --out-link ./result-$case ".#t1.$_CFG.ip.cases.$case.emu-result"
}
...
main() {
  ...
  info2 "* test: run test case with specific emulator"
