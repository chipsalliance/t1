...
assert
  if emulator.enableTrace then
    (lib.assertMsg (waveFileName != null) "waveFileName shall be set for trace build")
  else
    (lib.assertMsg (waveFileName == null) "waveFileName must not be set for non-trace build");
let
...
stdenvNoCC.mkDerivation (
  lib.recursiveUpdate {
    name =
      "${testCase.pname}-${emulator.emuKind}-result" + (lib.optionalString emulator.enableTrace "-trace");
    nativeBuildInputs = [
      zstd
      jq
      python3
    ...
    emuDriverWithArgs =
      assert lib.assertMsg (!(emulator.enableProfile or false))
        "The provided emulator has `profile` feature enable, which is inherently nondetermistic, use '.<...attr path...>.profile --impure' instead";
