...
struct VRFPerf {
  ...
  void print_summary(std::ostream &os) const {
    os << fmt::format("vrf_write_records: {}/{} ({:.3f}%)\n", vrf_write_records,
                      total_records,
                      (double)vrf_write_records / (double)total_records * 100.);
  }
...
struct ALUPerf {
  ...
  void print_summary(std::ostream &os) const {
    os << fmt::format(
        ...
        "divider_occupied: {}/{} ({:.3f}%)\n",
        adder_occupied_records, total_records,
        (double)adder_occupied_records / (double)total_records * 100.,
        shifter_occupied_records, total_records,
        (double)shifter_occupied_records / (double)total_records * 100.,
        multiplier_occupied_records, total_records,
        (double)multiplier_occupied_records / (double)total_records * 100.,
        divider_occupied_records, total_records,
        (double)divider_occupied_records / (double)total_records * 100.);
  }

  void step(int lane_idx, bool is_adder_occupied, bool is_shifter_occupied,
...
struct LSUPerf {
  ...
  void print_summary(std::ostream &os, int bank_idx) const {
    os << fmt::format("bank_{}_avg_pending_req: {}/{} ({:.3f})\n"
                      "bank_{}_occupied: {}/{} ({:.3f}%)\n",
                      bank_idx, total_pending_reqs, total_records,
                      (double)total_pending_reqs / (double)total_records,
                      bank_idx, total_occupied_lsu, total_records,
                      (double)total_occupied_lsu / (double)total_records *
                          100.);
  }
...
struct ChainingPerf {
  ...
  void print_summary(std::ostream &os) const {
    os << fmt::format("chaining_size: {}/{} ({:.3f}%)\n", total_occupied,
                      total_records,
                      (double)total_occupied / (double)total_records * 100.);
  }
