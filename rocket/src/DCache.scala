...
class DCacheErrors(implicit p: Parameters) extends L1HellaCacheBundle()(p) with CanHaveErrors {
  ...
  val rdata = for ((array, i) <- data_arrays.zipWithIndex) yield {
    ...
    when(valid && io.req.bits.write) {
      val wMaskSlice = (0 until wMask.size)
        .filter(j => i % (wordBits / subWordBits) == (j % (wordBytes / eccBytes)) / (subWordBytes / eccBytes))
        .map(wMask(_))
...
class DCacheModule(outer: DCache) extends HellaCacheModule(outer) {
  ...
  class DCacheModuleImpl { // entering gated-clock domain
    ...
    val tlb = Module(
      new TLB(
        ...
        TLBConfig(nTLBSets, nTLBWays, cacheParams.nTLBBasePageSectors, cacheParams.nTLBSuperpages)
      )
    )
    ...
    val s2_data = {
      ...
      if (cacheParams.pipelineWayMux) {
        ...
        val s1_word_en = Mux(
          !io.cpu.replay_next,
          word_en,
          UIntToOH(uncachedResp.addr.extract(log2Up(rowBits / 8) - 1, log2Up(wordBytes)), wordsPerRow)
        )
        (for (i <- 0 until wordsPerRow) yield {
  ...
  def encodeData(x: UInt, poison: Bool) =
    x.grouped(eccBits).map(dECC.encode(_, if (dECC.canDetect) poison else false.B)).asUInt
