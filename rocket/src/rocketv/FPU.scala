...
import chisel3.util.{BitPat, Cat, Decoupled, RegEnable, log2Ceil}
...
//    val sfma = Module(new FPUFMAPipe(cfg.sfmaLatency, FType.S))
    val sfma = Instantiate(new FPUFMAPipe(FPUFMAPipeParameter(cfg.sfmaLatency, FType.S)))
    sfma.io.in.valid := req_valid && ex_ctrl.fma && ex_ctrl.typeTagOut === S
    sfma.io.in.bits := fuInput(Some(sfma.t))

//    val fpiu = Module(new FPToInt)
    val fpiu = Instantiate(new FPToInt(FPToIntParameter()))
...
//    val ifpu = Module(new IntToFP(2))
    val ifpu = Instantiate(new IntToFP(IntToFPParameter(2)))
...
//    val fpmu = Module(new FPToFP(2))
    val fpmu = Instantiate(new FPToFP(FPToFPParameter(2)))
    ...
    case class Pipe[T <: Module](p: Instance[T], lat: Int, cond: (FPUCtrlSigs) => Bool, res: FPResult)
    val dfma = Option.when(fLen > 32)(Instantiate(new FPUFMAPipe(FPUFMAPipeParameter(cfg.dfmaLatency, FType.D))))
    dfma.foreach{ dfma =>
    ...
    val hfma = Option.when(minFLen == 16)(Instantiate(new FPUFMAPipe(FPUFMAPipeParameter(cfg.sfmaLatency, FType.H))))
    val pipes = List(
      Pipe(fpmu, fpmu.latency, (c: FPUCtrlSigs) => c.fastpipe, fpmu.io.out.bits),
      Pipe(ifpu, ifpu.latency, (c: FPUCtrlSigs) => c.fromint, ifpu.io.out.bits),
      Pipe(sfma, sfma.latency, (c: FPUCtrlSigs) => c.fma && c.typeTagOut === S, sfma.io.out.bits)) ++
      dfma.map(dfma => Pipe(dfma, dfma.latency, (c: FPUCtrlSigs) => c.fma && c.typeTagOut === D, dfma.io.out.bits)) ++
      hfma.map(hfma => Pipe(hfma, hfma.latency, (c: FPUCtrlSigs) => c.fma && c.typeTagOut === H, hfma.io.out.bits))
...
//    ccover(mem_reg_valid && write_port_busy, "WB_STRUCTURAL", "structural hazard on writeback")
    ...
    io.core.sboard_set := wb_reg_valid && !wb_cp_valid && RegNext(useScoreboard(_._1.cond(mem_ctrl)) || mem_ctrl.div || mem_ctrl.sqrt)
...
//    ccover(io.sboard_clr && load_wb, "DUAL_WRITEBACK", "load and FMA writeback on same cycle")
    ...
    io.core.illegal_rm := isOneOf(io.core.inst(14,12), Seq(5.U, 6.U)) || io.core.inst(14,12) === 7.U && io.core.fcsr_rm >= 5.U
...
class FPUDecoder(val parameter: FPUParameter)
  ...
  val insns = (parameter.minFLen, parameter.fLen) match {
    ...
    case other => throw new Exception(s"minFLen = ${parameter.minFLen} & fLen = ${parameter.fLen} is an unsupported configuration")
  }
  val decoder = DecodeLogic(io.inst, default, insns)
  val s = io.sigs
  val sigs = Seq(s.ldst, s.wen, s.ren1, s.ren2, s.ren3, s.swap12, s.swap23, s.typeTagIn, s.typeTagOut, s.fromint, s.toint, s.fastpipe, s.fma, s.div, s.sqrt, s.wflags)
