...
import chisel3.util.{BitPat, Cat, Fill, Pipe, Valid, log2Ceil}

case class PipelinedMultiplierParameter() extends SerializableModuleParameter {
  val latency: Int = ???
  val width: Int = ???
  val nXpr: Int = ???
  val uopWidth: Int = 4
...
class PipelinedMultiplierInterface(parameter: PipelinedMultiplierParameter) extends Bundle {
  val req = Flipped(Valid(new MultiplierReq(parameter.width, log2Ceil(parameter.nXpr), parameter.uopWidth)))
  val resp = Valid(new MultiplierResp(parameter.width, log2Ceil(parameter.nXpr)))
}
...
class PipelinedMultiplier(val parameter: PipelinedMultiplierParameter)
  ...
  val prod = lhs * rhs
  val muxed = Mux(cmdHi, prod(2 * width - 1, width), Mux(cmdHalf, sextTo(prod(width / 2 - 1, 0), width), prod(width - 1, 0)))
