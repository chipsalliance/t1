...
class TLBParameter() extends SerializableModuleParameter {
  ...
  val lgMaxSize:               Int = ???
  val asidBits:                Int = ???
  val usingHypervisor:         Boolean = ???
  val usingAtomics:            Boolean = ???
  val usingDataScratchpad:     Boolean = ???
  val usingAtomicsOnlyForIO:     Boolean = ???
  val usingVM:                 Boolean = ???
...
class TLB(val parameter: TLBParameter)
  ...
  val instruction = parameter.instruction
  ...
  val memIdx = vpn(log2Ceil(cfg.nSectors) + log2Ceil(cfg.nSets) - 1, log2Ceil(cfg.nSectors))
  ...
  val mpu_ppn = Mux[UInt](
    ...
    Mux(
      ...
      special_entry.map(e => TLBEntry.ppn(e, vpn, TLBEntry.getData(e, vpn), superpage = true, usingVM, pgLevelBits, superpageOnly = false)).getOrElse(0.U),
...
//  val legal_address: Bool = edge.manager.findSafe(mpu_physaddr).reduce(_ || _)
  val legal_address: Bool = ???
  ...
  val superpage_hits = superpage_entries.map(tlbEntry => TLBEntry.hit(tlbEntry, vpn, priv_v, superpage = true, usingVM: Boolean, pgLevelBits: Int, hypervisorExtraAddrBits: Int, superpageOnly = true))
  val hitsVec = VecInit(all_entries.map(tlbEntry => vm_enabled && TLBEntry.hit(tlbEntry, vpn, priv_v, superpage = ???, usingVM: Boolean, pgLevelBits: Int, hypervisorExtraAddrBits: Int, superpageOnly = ???)))
  ...
  when(do_refill) {
    ...
    when(special_entry.nonEmpty.B && !io.ptw.resp.bits.homogeneous) {
      special_entry.foreach(tlbEntry => TLBEntry.insert(tlbEntry, r_refill_tag, refill_v, io.ptw.resp.bits.level, newEntry, superpageOnly = ???))
      ...
      for ((e, i) <- superpage_entries.zipWithIndex) when(r_superpage_repl_addr === i.U) {
        TLBEntry.insert(e, r_refill_tag, refill_v, io.ptw.resp.bits.level, newEntry, superpageOnly = ???)
      ...
      for ((e, i) <- sectored_entries(r_memIdx).zipWithIndex) when(waddr === i.U) {
        ...
        TLBEntry.insert(e, r_refill_tag, refill_v, 0.U, newEntry, superpageOnly = ???)
  ...
  val ppn = Mux1H(
    hitsVec :+ !vm_enabled,
    (all_entries.zip(entries)).map { case (entry, data) => TLBEntry.ppn(entry, vpn, data, superpage = ???, usingVM: Boolean, pgLevelBits: Int, superpageOnly = ???) } :+ vpn(ppnBits - 1, 0)
  )
  ...
  if (usingVM) {
    ...
    when(sfence) {
      ...
      for (e <- all_real_entries) {
        ...
        when(!hg && io.sfence.bits.rs1) { TLBEntry.invalidateVPN(e, vpn, hv, superpage = ???, usingVM, pgLevelBits, hypervisorExtraAddrBits, superpageOnly = ???) }
...
//    ccover(multipleHits, "MULTIPLE_HITS", "Two matching translations in TLB")
  ...
  def replacementEntry(set: Seq[TLBEntry], alt: UInt) = {
    val valids = VecInit(set.map(tlbEntry => VecInit(tlbEntry.valid).asUInt.orR)).asUInt
