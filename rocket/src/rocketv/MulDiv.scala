...
import chisel3.util.{BitPat, Cat, Decoupled, Enum, Fill, Log2, Pipe, Valid, log2Ceil, log2Floor}
...
case class MulDivParameter() extends SerializableModuleParameter {
  ...
  val divUnroll: Int = ???
  val divEarlyOut: Boolean = ???
  val divEarlyOutGranularity: Int = ???
  val mulUnroll: Int = ???
  val mulEarlyOut: Boolean = ???
...
class MulDivInterface(parameter: MulDivParameter) extends Bundle {
  val req = Flipped(Decoupled(new MultiplierReq(parameter.width, log2Ceil(parameter.nXpr), parameter.uopWidth)))
  val kill = Input(Bool())
  val resp = Decoupled(new MultiplierResp(parameter.width, log2Ceil(parameter.nXpr)))
}
...
class MulDiv(val parameter: MulDivParameter)
  ...
  private def minDivLatency = Option.when(cfg.divUnroll > 0)(if (cfg.divEarlyOut) 3 else 1 + w / cfg.divUnroll)
  private def minMulLatency = Option.when(cfg.mulUnroll > 0)(if (cfg.mulEarlyOut) 2 else w / cfg.mulUnroll)
