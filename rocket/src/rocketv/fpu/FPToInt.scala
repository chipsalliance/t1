...
import chisel3.experimental.hierarchy.{Instance, instantiable}
import chisel3.util.{BitPat, Cat, Decoupled, Fill, Pipe, RegEnable, Valid, log2Ceil}
import chisel3.util.circt.ClockGate


case class FPToIntParameter() extends SerializableModuleParameter {
  val latency: Int = ???
  val fLen: Int = ???
  val xLen: Int = ???
  val minFLen: Int = ???
...
class FPToInt(val parameter: FPToIntParameter)
  ...
  val store = VecInit(floatTypes.map(t =>
    if (t == FType.H) Fill(maxType.ieeeWidth / minXLen,  sextTo(ieee(in.in1)(15, 0), minXLen))
    else Fill(maxType.ieeeWidth / t.ieeeWidth, ieee(in.in1)(t.ieeeWidth - 1, 0))))(tag)
  val toint = WireDefault(store)
  val intType = WireDefault(in.fmt(0))
  io.out.bits.store := store
  io.out.bits.toint := VecInit((0 until helper.nIntTypes).map(i => sextTo(toint((helper.minXLen << i) - 1, 0), xLen)): Seq[UInt])(intType)
  io.out.bits.exc := 0.U
