...
class ICacheInterface(parameter: ICacheParameter) extends Bundle {
  ...
  val errors = new ICacheErrors(parameter.hasCorrectable, parameter.hasUncorrectable, parameter.paddrBits)
  ...
  val instructionFetchAXI: AXI4ROIrrevocable = org.chipsalliance.amba.axi4.bundle.chisel.irrevocable(parameter.instructionFetchParameter)

  val itimAXI: Option[AXI4RWIrrevocable] = parameter.itimParameter.map(Flipped(org.chipsalliance.amba.axi4.bundle.chisel.irrevocable(_)))
}
...
class ICache(val parameter: ICacheParameter)
  ...
  /** scratchpad base address, if exist [[ICacheParams.itimAddr]], add [[ReplicatedRegion]] to base.
    ...
    * should be a Int
    */
  val scratchpadBase: Option[UInt] = ???
