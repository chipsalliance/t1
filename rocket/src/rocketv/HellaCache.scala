...
import chisel3.util.{Arbiter, Cat, Enum, Fill, FillInterleaved, Mux1H, MuxLookup, OHToUInt, PriorityEncoder, PriorityEncoderOH, PriorityMux, RegEnable, SRAM, SRAMInterface, UIntToOH, isPow2, log2Ceil}
...
case class HellaCacheParameter() extends SerializableModuleParameter {
  ...
  val dtimParameter: Option[AXI4BundleParameter] = ???
  val instructionFetchParameter: AXI4BundleParameter = ???
  val coreDataBits: Int = xLen max fLen

  val tagCode: Code
  val dataCode: Code
  val silentDrop: Boolean
  val acquireBeforeRelease: Boolean
  val clockGate: Boolean
  val replacementPolicy: String
  val idxMSB: Int
  val idxLSB: Int
  val subWordBits: Int
  val rowWords: Int
...
class HellaCacheInterface(parameter: HellaCacheParameter) extends Bundle {
  ...
  val tlb_port = new DCacheTLBPort(parameter.paddrBits, parameter.vaddrBitsExtended, parameter.coreDataBytes)
  val loadStoreAXI: AXI4RWIrrevocable = org.chipsalliance.amba.axi4.bundle.chisel.irrevocable(parameter.instructionFetchParameter)
  val dtimAXI: Option[AXI4RWIrrevocable] = parameter.dtimParameter.map(Flipped(org.chipsalliance.amba.axi4.bundle.chisel.irrevocable(_)))
}
...
class HellaCache(val parameter: HellaCacheParameter)
  ...
  object cacheParams {
    def tagCode = parameter.tagCode
    def dataCode = parameter.dataCode
    def silentDrop = parameter.silentDrop
    def acquireBeforeRelease = parameter.acquireBeforeRelease
    def clockGate = parameter.clockGate
    def replacementPolicy = parameter.replacementPolicy
    def separateUncachedResp = parameter.separateUncachedResp
    def pipelineWayMux = parameter.pipelineWayMux
  }
  def rowWords = parameter.rowWords
  def wordOffBits = parameter.wordOffBits
  def beatWords = parameter.beatWords
  def beatBytes = parameter.beatBytes
  def idxMSB = parameter.idxMSB
  ...
  def pgLevelBits = parameter.pgLevelBits
  def dcacheReqTagBits = parameter.dcacheReqTagBits
  def dcacheArbPorts = parameter.dcacheArbPorts
  def coreDataBytes = parameter.coreDataBytes
  def encBits = parameter.encBits
  def untagBits = parameter.untagBits
  def rowBytes = parameter.rowBytes
  def subWordBytes = parameter.subWordBytes
  def rowOffBits = parameter.rowOffBits
  def beatOffBits = parameter.beatOffBits
  def wordBytes = parameter.wordBytes
  def usingAtomicsInCache = parameter.usingAtomicsInCache
  def nWays = parameter.nWays
  def nSets = parameter.nSets
  def cacheBlockBytes = parameter.cacheBlockBytes
  def vaddrBitsExtended = parameter.vaddrBitsExtended
  def paddrBits = parameter.paddrBits
  def maxUncachedInFlight = parameter.maxUncachedInFlight
  def tagBits = parameter.tagBits
  def idxBits = parameter.idxBits
  def blockOffBits = parameter.blockOffBits
  def usingDataScratchpad = parameter.usingDataScratchpad
  def usingAtomics = parameter.usingAtomics
  def lrscBackoff = parameter.lrscBackoff
  def lrscCycles = parameter.lrscCycles
  def rowBits = parameter.rowBits
  def cacheDataBits = parameter.rowBits
  ...
  def wordBits = parameter.coreDataBits
  object outer {
  ...
  object ClientMetadata {
    // MI
    val maximum = ClientMetadata(0.U(3.W))
    val onReset = ClientMetadata(0.U(2.W))
    def isValid(cm: ClientMetadata): Bool = ???
  ...
  object L1Metadata {
    def apply(tag: Bits, coh: ClientMetadata) = {
      val meta = Wire(new L1Metadata(2))
      meta.tag := tag
      meta.coh := coh
      meta
    }
    ...
    val s2_data = {
      ...
      if (cacheParams.pipelineWayMux) {
        ...
        val s1_word_en = Mux(
          !io.cpu.replay_next,
          word_en,
          UIntToOH(uncachedResp.addr.extract(log2Ceil(rowBits / 8) - 1, log2Ceil(wordBytes)), wordsPerRow)
        )
        (for (i <- 0 until wordsPerRow) yield {
...
//    val (s2_hit, s2_grow_param, s2_new_hit_state) = s2_hit_state.onAccess(s2_req.cmd)
    ...
    val s2_word_idx = s2_req.addr(log2Ceil(rowBits / 8) - 1, log2Ceil(wordBytes))
    ...
    val a_mask = pstore1_mask << (access_address(log2Ceil(beatBytes) - 1, log2Ceil(wordBytes)) << 3)
...
//    metaArb.io.in(3).valid := grantIsCached && d_done && !tl_out.d.bits.denied
    ...
    metaArb.io.in(3).bits.data := tECC.encode(
      L1Metadata(s2_req.addr >> tagLSB, ClientMetadata(nextState(s2_req.cmd))).asUInt
    )
    ...
    when(s2_release_data_valid) {
      ...
      io.loadStoreAXI.w.bits.strb := -1.S(io.loadStoreAXI.w.bits.strb.getWidth.W)
    ...
    val s1_uncached_data_word = {
      val word_idx = uncachedResp.addr(log2Ceil(rowBits / 8) - 1, log2Ceil(wordBytes))
      val words = grouped(io.loadStoreAXI.r.bits.data, wordBits)
      words(word_idx)
    }
...
//    }
  ...
  def isRead(op: UInt): Bool = ???
  def isWrite(op: UInt): Bool = ???
