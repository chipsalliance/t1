...
case class CSRParameter(xLen: Int, usingSupervisor: Boolean, nLocalInterrupts: Int, hartIdLen: Int, pgLevels: Int, minPgLevels: Int, maxPAddrBits: Int, vLen: Int, customCSRSize: Int) extends SerializableModuleParameter {
  // compatibility mode
  val decodeWidth = 1
  // @todo remove BEU for now
  val hasBeu = false
  ...
  val usingFPU = false
  val haveBasicCounters = false
  val resetVectorLen: Int = ???
  val iLen = 32
  val vaddrBitsExtended: Int = ???
  val nDscratch: Option[Int] = ???
  val retireWidth: Int = 1
  val maxSVAddrBits: Int = ???
  ...
  val usingVector: Boolean = ???
  val usingCompressed: Boolean = ???
  val misaWritable: Boolean = ???
  val mtvecWritable: Boolean = ???
  val customIsaExt: Option[String] = ???
  val usingAtomics: Boolean = ???
  val usingDebug: Boolean = ???
  val usingMulDiv: Boolean = ???
  val useRVE: Boolean = ???
  val nPerfCounters: Int = ???
  val fLen: Int = ???
  val usingUser: Boolean = ???
  val debugEntry: Option[BigInt] = ???
  val debugException: Option[BigInt] = ???
  // TODO: use layer for DV
  val enableCommitLog: Boolean = false
...
class CSRInterface(parameter: CSRParameter) extends Bundle {
  ...
  val reset = Input(Clock())
  val ungatedClock = Input(Clock())
  val interrupts = Input(new CoreInterrupts(parameter.usingSupervisor, parameter.nLocalInterrupts, parameter.hasBeu, parameter.usingNMI, parameter.resetVectorLen))
  ...
  val bp = Output(Vec(parameter.nBreakpoints, new BP(parameter.xLen, parameter.useBPWatch, parameter.vaddrBits, parameter.mcontextWidth, parameter.scontextWidth)))
  ...
  val wbRegRS2 = Option.when(parameter.usingVector)(Input(UInt()))
  // @todo custom CSR
...
class CSR(val parameter: CSRParameter)
  ...
  val clock = io.clock
  val reset = io.reset
  val perfEventSets: EventSets = ???
  val customCSRs:    Seq[CustomCSR] = ???
  val hasBeu: Boolean = ???
  // compatibility mode
  // TODO: remove me.
  ...
  val reg_bp = Reg(Vec(1 << log2Up(nBreakpoints), new BP(xLen, useBPWatch, vaddrBits, coreParams.mcontextWidth, coreParams.scontextWidth)))
  ...
  val reg_instret = WideCounter(64, io.retire, inhibit = reg_mcountinhibit(2)).value
  val reg_cycle =
    if (enableCommitLog) WideCounter(64, io.retire, inhibit = reg_mcountinhibit(0)).value
    else withClock(io.ungatedClock) { WideCounter(64, !io.csrStall, inhibit = reg_mcountinhibit(0)).value }
  val reg_hpmevent = io.counters.map(c => RegInit(0.U(xLen.W)))
  (io.counters.zip(reg_hpmevent)).foreach { case (c, e) => c.eventSel := e }
  val reg_hpmcounter = io.counters.zipWithIndex.map {
    case (c, i) =>
      WideCounter(CSR.hpmWidth, c.inc, reset = false, inhibit = reg_mcountinhibit(CSR.firstHPM + i)).value
  }
  ...
  val d_interrupts: UInt = io.interrupts.debug << CSR.debugIntCause
  val (nmi_interrupts: UInt, nmiFlag: Bool) = io.interrupts.nmi
    .map(nmi =>
      (
  ...
  if (coreParams.haveBasicCounters) {
    ...
    for (
      ((e, c), i) <- (reg_hpmevent
        .padTo(CSR.nHPM, 0.U)
        .zip(reg_hpmcounter.map(x => x: UInt).padTo(CSR.nHPM, 0.U)))
        .zipWithIndex
    ) {
  ...
  if (reg_pmp.nonEmpty) {
    ...
    for (i <- 0 until read_pmp.size by pmpCfgPerCSR)
      read_mapping += (CSRs.pmpcfg0 + pmpCfgIndex(i)) -> Cat(read_pmp.map(_.cfg).slice(i, i + pmpCfgPerCSR).reverse.map(_.asUInt)).asUInt
  ...
  for (io_dec <- io.decode) {
    ...
    def decodeAny(m: mutable.LinkedHashMap[Int, Bits]): Bool = m.map { case (k: Int, _: Bits) => addr === k.U }.reduce(_ || _)
    ...
    val is_counter = inRange(addr, CSR.firstCtr.U, (CSR.firstCtr + CSR.nCtr).U) || inRange(addr,
      CSR.firstCtrH.U,
...
//  })
  ...
  io.csrwCounter := Mux(
    coreParams.haveBasicCounters.B && csr_wen && inRange(io.rw.addr,
      CSRs.mcycle.U,
  ...
  when(csr_wen) {
    ...
    if (usingHypervisor) {
      ...
      when(decoded_addr(CSRs.hip)) {
        val new_hip = ((read_mip & ~hs_delegable_interrupts) | (wdata & hs_delegable_interrupts)).asTypeOf(new MIP(nLocalInterrupts))
      ...
      when(decoded_addr(CSRs.hvip)) {
        val new_sip = ((read_mip & ~hs_delegable_interrupts) | (wdata & hs_delegable_interrupts)).asTypeOf(new MIP(nLocalInterrupts))
  ...
  for (pmp <- reg_pmp) {
    ...
    when(reset.asBool) { resetPMP(pmp) }
  }
  ...
  def formTVec(x:        UInt): UInt = andNot(x, Mux(x(0), ((((BigInt(1) << mtvecInterruptAlign) - 1) << mtvecBaseAlign) | 2).U, 2.U))
