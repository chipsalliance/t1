...
class LaneZvbbProbe(parameter: LaneZvbbParam)
    extends DVBundle[LaneZvbbParam, LaneZvbbLayers](parameter)
...
object LaneZvbb extends Generator[LaneZvbbParam, LaneZvbbLayers, LaneZvbbInterface, LaneZvbbProbe]:
  ...
  def architecture(parameter: LaneZvbbParam) =
    ...
    val zvbbBRev = Seq.tabulate(p.datapathWidth)(i => zvbbSrc.asBits.bit(i).asBits).reduce((a: Referable[Bits], b: Referable[Bits]) => a ## b).asUInt
    ...
    // SLL64
    val zvbbSLL64_32 = ((zero32.asBits ## zvbbSrc.asBits).asUInt << zvbbRs.asBits.bits(4, 0).asUInt).asBits.bits(31, 0).asUInt
    val zvbbSLL64_16 =
      val sll64_16a = ((zero16.asBits ## zvbbSrc16a.asBits).asUInt << zvbbRs16a.asBits.bits(3, 0).asUInt).asBits.bits(15, 0).asUInt
      val sll64_16b = ((zero16.asBits ## zvbbSrc16b.asBits).asUInt << zvbbRs16b.asBits.bits(3, 0).asUInt).asBits.bits(15, 0).asUInt
      (sll64_16a.asBits ## sll64_16b.asBits).asUInt
    val zvbbSLL64_8 =
      val sll64_8a = ((zero8.asBits ## zvbbSrc8a.asBits).asUInt << zvbbRs8a.asBits.bits(2, 0).asUInt).asBits.bits(7, 0).asUInt
      val sll64_8b = ((zero8.asBits ## zvbbSrc8b.asBits).asUInt << zvbbRs8b.asBits.bits(2, 0).asUInt).asBits.bits(7, 0).asUInt
      val sll64_8c = ((zero8.asBits ## zvbbSrc8c.asBits).asUInt << zvbbRs8c.asBits.bits(2, 0).asUInt).asBits.bits(7, 0).asUInt
      val sll64_8d = ((zero8.asBits ## zvbbSrc8d.asBits).asUInt << zvbbRs8d.asBits.bits(2, 0).asUInt).asBits.bits(7, 0).asUInt
    ...
    val zvbbPOP =
      ...
      mux1H(
        ...
        Seq(
          ...
          (0.U(8).asBits ## (zvbbPOP8a + zvbbPOP8b).asBits.bits(7, 0) ## 0.U(8).asBits ## (zvbbPOP8c + zvbbPOP8d).asBits.bits(7, 0)).asUInt,
