...
@generator
object LaneShifter
    extends Generator[LaneShifterParameter, LaneShifterLayers, LaneShifterInterface, LaneShifterProbe]:
  ...
  def architecture(parameter: LaneShifterParameter) =
    ...
    // v[d - 2 : 0]
    val vLostLSB = ((roundMask >> 1.U(1)).asBits.bits(p.datapathWidth - 1, 0).asUInt.asBits & shifterSource.asBits).asUInt.asBits.orR
    // v[d]
