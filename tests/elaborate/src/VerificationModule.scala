...
class VerificationModule(dut: V) extends TapModule {
  ...
  req.valid := dpiPokeInst.instValid
  ...
  dpiPeekIssue.issueIdx := tap(dut.instCount)
  ...
  class PokeTL(bundle: TLBundle) extends ExtModule with HasExtModuleInline {
    ...
    @public val dReady = IO(Output(bundle.d.ready))
    setInline("dpiPokeTL.sv",
      s"""module dpiPokeTL(
         ...
         |  input bit d_ready
         |);
         |always @ (posedge clock) #($latPokeTL) dpiPokeTL(
         |  channel,
         |  dBits_opcode,
         |  dBits_param,
         |  dBits_size,
