...
rec {
  ...
  t1rocket-check = lib.overrideDerivation verilator-check (old: {
    name = old.name + "-t1rocket";
    emuDriver = "${t1rocket-emu}/bin/online_drive";
  });

  t1rocket-check-trace = lib.overrideDerivation t1rocket-check (old: {
    name = old.name + "-with-trace";
    emuDriver = "${verilator-emu-trace}/bin/online_drive";
    emuDriverArgs = old.emuDriverArgs or [ ] ++ [ "--wave-path" "${placeholder "out"}/wave.fst" ];
    postCheck = ''
