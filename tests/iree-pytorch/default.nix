...
let
  ...
  build = { caseName, sourcePath }:
    builder {
      ...
      nativeBuildInputs = [ iree pythonEnv ];
      ...
      ireeCEmbedDataArgs = [
        "--output_header=matmul_module_dylib_riscv_32.h"
        "--output_impl=matmul_module_dylib_riscv_32.c"
        "--identifier=iree_mlir_test_matmul_module_dylib_riscv_32"
        "--flatten"
      ];
      ...
      buildPhase = ''
        ...
        iree-compile $ireeCompileArgs matmul.mlir -o matmul_module_dylib_riscv_32.vmfb

        iree-c-embed-data $ireeCEmbedDataArgs matmul_module_dylib_riscv_32.vmfb

        $CC -c -fPIC matmul_module_dylib_riscv_32.c -o matmul_module_dylib.o

        $CC -c -fPIC $ireeCFlags init.c -o matmul_init.o

        $CC -c -fPIC $ireeCFlags device_embedded_sync.c -o device_embedded_sync.o

        $CC -T${linkerScript} ${t1main} matmul_init.o matmul_module_dylib.o device_embedded_sync.o \
          -liree_vm_impl \
          -liree_base_base \
