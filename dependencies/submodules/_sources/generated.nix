# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  arithmetic = {
    pname = "arithmetic";
    version = "3ee6a01d3a078782463ac337561f4949906d500e";
    src = fetchFromGitHub {
      owner = "sequencer";
      repo = "arithmetic";
      rev = "3ee6a01d3a078782463ac337561f4949906d500e";
      fetchSubmodules = false;
      sha256 = "sha256-TDYJMD5BGuROGHc3lOsEG0aFQDXlVXWB0xZ1pMcS7g0=";
    };
    date = "2025-03-20";
  };
  chisel = {
    pname = "chisel";
    version = "ff99b21a2f88bef85f07f4a6bb373ed4c174de5b";
    src = fetchFromGitHub {
      owner = "chipsalliance";
      repo = "chisel";
      rev = "ff99b21a2f88bef85f07f4a6bb373ed4c174de5b";
      fetchSubmodules = false;
      sha256 = "sha256-n1ps5+X84eL60+15007wpNz7yg5Nh0R8YbhwejuTvTU=";
    };
    date = "2025-05-19";
  };
  chisel-interface = {
    pname = "chisel-interface";
    version = "a61c4d8dbbe25da7c267d03e2be0e24638258023";
    src = fetchFromGitHub {
      owner = "chipsalliance";
      repo = "chisel-interface";
      rev = "a61c4d8dbbe25da7c267d03e2be0e24638258023";
      fetchSubmodules = false;
      sha256 = "sha256-cQ0ybo4sxt5X3UOH5NT1f8SmXV+4zeaTo/0zUIr/Kzg=";
    };
    date = "2025-03-20";
  };
  riscv-opcodes = {
    pname = "riscv-opcodes";
    version = "d0c4b0b4ef4c56fbd0499b010342b8107585233e";
    src = fetchFromGitHub {
      owner = "riscv";
      repo = "riscv-opcodes";
      rev = "d0c4b0b4ef4c56fbd0499b010342b8107585233e";
      fetchSubmodules = false;
      sha256 = "sha256-vJigg7GZDUWHmYTc4dsCon7JBs6IW1yY1GRPLfBqkMc=";
    };
    date = "2025-03-19";
  };
  rvdecoderdb = {
    pname = "rvdecoderdb";
    version = "c7acc990037e6b984ab6defb65fdc82f9151c656";
    src = fetchFromGitHub {
      owner = "sequencer";
      repo = "rvdecoderdb";
      rev = "c7acc990037e6b984ab6defb65fdc82f9151c656";
      fetchSubmodules = false;
      sha256 = "sha256-iGZiYBDaUIej9iYAtHWsINAq2LzETKhiQqJB8buA1v4=";
    };
    date = "2025-03-21";
  };
  zaozi = {
    pname = "zaozi";
    version = "49c207170f73701d8aef0efaad4b3ff8f3e64b81";
    src = fetchFromGitHub {
      owner = "sequencer";
      repo = "zaozi";
      rev = "49c207170f73701d8aef0efaad4b3ff8f3e64b81";
      fetchSubmodules = false;
      sha256 = "sha256-26RcNx5eujUTSLwTTguDoPai28u+0cXue3qFvB37alg=";
    };
    date = "2025-05-20";
  };
}
