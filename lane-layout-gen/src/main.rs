...
fn main() {
    ...
    let mut searcher = LayoutSearcher::new(args.n);
    ...
    eprintln!("find a layout with the max distance: {}", searcher.best_d);
    if args.print_json {
...
impl LayoutSearcher {
    fn new(n: u64) -> Self {
        ...
        Self{n, best_d: i64::MAX, best_layout: None, nodes}
    }
        ...
        for (count, shuffle_idx) in idx_permu.iter().enumerate() {
            let pos = if count == 0 {
                ...
                if placed_neighbors_pos.is_empty() {
                    ...
                    let y_range = Uniform::from(ymin - enlarge..ymax + enlarge + 1);
                    loop {
