...
impl ShadowDevice for DisplayDevice {
  ...
  fn read_mem(&self, addr: usize, size: usize) -> Vec<u8> {
    ...
    let vram_part_end = std::cmp::min(REG_FLUSH, end);
    let vram_part_size = if vram_part_end > start {
      vram_part_end - start
    } else {
      0
    };

    let reg_part_start = std::cmp::max(REG_FLUSH, start);
    let reg_part_size = if end > reg_part_start {
      end - reg_part_start
    } else {
      0
    };
    ...
    assert_eq!(
      vram_part_size + reg_part_size,
      size,
      "Size calculation done wrong?"
    );

    self.vram_read(&mut result[0..vram_part_size], start, vram_part_size);
    self.reg_read(
      &mut result[vram_part_size..size],
      reg_part_start,
      reg_part_size,
    );
...
impl DisplayDevice {
  fn vram_read(&self, buffer: &mut [u8], offset: usize, size: usize) {
    let start = std::cmp::min(self.vram.len(), offset);
    let end = std::cmp::min(self.vram.len(), offset + size);
    buffer[0..(start - end)].copy_from_slice(&self.vram[start..end]);
  }

  fn reg_read(&self, buffer: &mut [u8], offset: usize, size: usize) {
    ...
    buffer[0..(start - end)].copy_from_slice(&reg_space[start..end]);
  }
