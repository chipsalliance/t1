...
class LaneFloat(val parameter: LaneFloatParam) extends VFUModule with SerializableModule[LaneFloatParam] {
  ...
  val fmaIn2      = Mux(
    ...
    recIn0,
    Mux(
      maf && !request.floatMul,
      recIn2,
      Mux(
        maf && request.floatMul,
        ((request.src(0) ^ request.src(1)) & (BigInt(1) << (parameter.datapathWidth - 1)).U) << 1,
        recIn1
      )
    )
  )
  ...
  val compareNaN     = Mux(oneNaN, Mux(raw0.isNaN, request.src(1), request.src(0)), "x7fc00000".U)
  ...
  compareResult := Mux(
    ...
    Mux(
      uop === BitPat("b1?00"),
      Mux(
        (!uop(2) && compareModule.io.lt) || (uop(2) && compareModule.io.gt) || (differentZeros && (uop(2) ^ request
          .src(1)(31))),
        request.src(1),
        request.src(0)
      ),
