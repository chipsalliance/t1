...
import chisel3.util.{log2Ceil, BitPat, Decoupled, DecoupledIO, Mux1H, RegEnable, UIntToOH, Valid, ValidIO}
...
case class ZVMADataExchangeParam(
  chainingSize:     Int,
  datapathWidth:    Int,
  vLen:             Int,
  laneNumber:       Int,
...
class ZVMADataExchange(val parameter: ZVMADataExchangeParam)
  ...
  val lastVs1Group   =
    (lastVs1Byte >> groupByteBit).asUInt - (!changeUIntSize(lastVs1Byte, groupByteBit).orR && lastVs1Byte.orR)
  val lastVs2Group   =
    (lastVs2Byte >> groupByteBit).asUInt - (!changeUIntSize(lastVs2Byte, groupByteBit).orR && lastVs2Byte.orR)
  ...
  Seq.tabulate(2) { i =>
    ...
    val dataVec: Vec[UInt] =
      RegInit(VecInit(Seq.tabulate(4)(_ => 0.U((parameter.datapathWidth * parameter.laneNumber).W))))
    ...
    deqData(i)      := Mux1H(
      ...
      Seq(
        VecInit(
          dataVec
            .map(s => Mux1H(UIntToOH(deqIndex), cutUIntBySize(s, 4)))
            .map(s => cutUInt(s, 8))
            .transpose
            .map(s => VecInit(s).asUInt)
        ).asUInt,
        VecInit(
          dataVec.init.init
            .map(s => Mux1H(UIntToOH(deqIndex(0)), cutUIntBySize(s, 2)))
            .map(s => cutUInt(s, 16))
            .transpose
            .map(s => VecInit(s).asUInt)
        ).asUInt,
  ...
  val storeDataAfterAlign: UInt =
    (((zvmaDataQueue.deq.bits ## shifterReg) << (addressInit ## 0.U(3.W))) >> parameter.dlen).asUInt
  val loadDataAfterAlign:  UInt =
    (((memDataQueue.deq.bits ## shifterReg) << (addressInit ## 0.U(3.W))) >> parameter.dlen).asUInt
