...
import org.chipsalliance.t1.rtl.{CSRInterface, LSURequest, VRFReadRequest, VRFWriteRequest, ffo, firstlastHelper}
...
class SimpleAccessUnit(param: MSHRParam) extends Module  with LSUPublic {
  ...
  val addressInBeatByte: UInt = s1Reg.address(log2Ceil(param.laneNumber * 4) - 1, 0)
  // 1 -> 1 2 -> 3 4 -> 15
  val baseMask: UInt = dataEEWOH(2) ## dataEEWOH(2) ## !dataEEWOH(0) ## true.B
  /** compute the mask for store transaction. */
  val storeMask: UInt = (baseMask << addressInBeatByte).asUInt(param.laneNumber * 4 - 1, 0)
  ...
  memReadRequest.bits.address := s1Reg.address
  memReadRequest.bits.size := dataEEW
  ...
  val offsetRecord = RegInit(VecInit(Seq.fill(memoryRequestSourceOH.getWidth)(0.U(log2Ceil(param.laneNumber * 4).W))))
