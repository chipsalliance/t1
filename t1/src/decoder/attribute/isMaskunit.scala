...
case class isMaskunit(value: TriState) extends BooleanDecodeAttribute {
  override val description: String = "mask unit -> red || compress || viota || ffo || slid || maskDestination || gather(v) || mv || popCount || extend all instruction in Sequencer mask unit. "
}
