...
class MaskExchangeUnit(parameter: LaneParameter) extends Module {
  ...
  val maskUnitResponsePipeType: LaneStage3Enqueue = WireInit(0.U.asTypeOf(enqueue.bits))
  val regEnq: Bool = (enqueue.fire && !enqIsMaskRequest) || maskUnitResponse.valid
  when(regEnq) {
    requestPipeReq := Mux(enqueue.fire, enqueue.bits, maskUnitResponsePipeType)
  }
