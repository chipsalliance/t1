...
class Lane(val parameter: LaneParameter) extends Module with SerializableModule[LaneParameter] {
  ...
  // segment ls type
  val segmentLS: Bool = laneRequest.bits.loadStore && laneRequest.bits.segment.orR && !laneRequest.bits.wholeRegisterLS
  // 0 -> 1, 1 -> 2, 2 -> 4, 4 -> 8
  ...
  val selectMask: UInt = Mux(
    ...
    segmentMask,
    Mux(
      laneRequest.bits.decodeResult(Decoder.nr) || laneRequest.bits.wholeRegisterLS,
      nrMask,
      lastWriteOH
    )
  )
