...
class WriteCheck(val parameter: VRFParam) extends Module {
  ...
  val hitVs2:      Bool =
    ((checkOH & maskForVs2) === 0.U || record.bits.unalignedReadVs2) && check.vd(4, 3) === record.bits.vs2(4, 3)
  val hitVs21:     Bool =
    ((checkOH & maskForVs21) === 0.U || record.bits.unalignedReadVs2) && check
      .vd(4, 3) === (record.bits.vs2(4, 3) + 1.U)
