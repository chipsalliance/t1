...
class VRF(val parameter: VRFParam) extends Module with SerializableModule[VRFParam] {
  ...
  writeCheck.zip(writeAllow).foreach{ case (check, allow) =>
    ...
    allow := chainingRecordCopy.map { record =>
      ...
      val notHitMaskVd = (checkOH & record.bits.elementMask) === 0.U
      ...
      val vs1Mask = (((-1.S(parameter.elementSize.W)).asUInt ## record.bits.elementMask) <<
