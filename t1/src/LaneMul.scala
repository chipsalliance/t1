...
case class LaneMulParam(eLen: Int, latency: Int, laneScale: Int)
...
class LaneMulReq(sourceWidth: Int) extends VFUPipeBundle {
  val src:      Vec[UInt] = Vec(3, UInt(sourceWidth.W))
...
class LaneMul(val parameter: LaneMulParam) extends VFUModule with SerializableModule[LaneMulParam] {
  ...
  val responseVec: Seq[(UInt, Bool)] = Seq.tabulate(parameter.laneScale) { index =>
    ...
    val attributeVec:            Seq[(Bool, UInt)] = adderResultVec.zipWithIndex.map { case (data, index) =>
      ...
      val operation0Sign    = (sourceSign0 && subRequest.sign) ^ negative
      val operation1Sign    = (sourceSign1 && subRequest.sign) ^ negative
