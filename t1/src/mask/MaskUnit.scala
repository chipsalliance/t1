...
class MaskUnitInterface(parameter: T1Parameter) extends Bundle {
  ...
  val instReq           = Flipped(Valid(new MaskUnitInstReq(parameter)))
  val exeReq            = Flipped(Vec(parameter.laneNumber, Valid(new MaskUnitExeReq(parameter.laneParam))))
  val exeResp           = Vec(
  ...
  val tokenIO           = Flipped(Vec(parameter.laneNumber, new LaneTokenBundle))
  val readChannel       = Vec(
    parameter.laneNumber,
    Decoupled(
      new VRFReadRequest(
  ...
  val readResult        = Flipped(Vec(parameter.laneNumber, Valid(UInt(parameter.datapathWidth.W))))
  val writeRD           = Valid(UInt(parameter.datapathWidth.W))
  val lastReport        = Output(UInt((2 * parameter.chainingSize).W))
  val lsuMaskInput      = Output(Vec(parameter.lsuMSHRSize, UInt(parameter.maskGroupWidth.W)))
  val lsuMaskSelect     = Input(Vec(parameter.lsuMSHRSize, UInt(parameter.lsuParameters.maskGroupSizeBits.W)))
  val laneMaskInput     = Output(Vec(parameter.laneNumber, UInt(parameter.datapathWidth.W)))
  val laneMaskSelect    = Input(Vec(parameter.laneNumber, UInt(parameter.laneParam.maskGroupSizeBits.W)))
  val laneMaskSewSelect = Input(Vec(parameter.laneNumber, UInt(2.W)))
  val v0UpdateVec       = Flipped(Vec(parameter.laneNumber, Valid(new V0Update(parameter.laneParam))))
  val writeRDData       = Output(UInt(parameter.xLen.W))
  val gatherData        = Decoupled(UInt(parameter.xLen.W))
  val gatherRead        = Input(Bool())
