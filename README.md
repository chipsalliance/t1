...
T1 supports standard `Zvf32` and `Zvx32`, `VLEN`/`DLEN` can be increased upto `64K`, hitting the RISC-V Vector architecture bottleneck.
...
T1 is designed to balance the throughput, area, and frequency among the VRF, VFU, and LSU. With T1 generator, it can be easily configured to achieve either high efficiency or high performance, depending on the desired trade-offs, even adding function units or purging out FPU, which supports `Zvf32` and remains `Zvx32` only.
