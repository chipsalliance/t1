...
pub struct VTable {
    ...
    data: &'static raw::pokedex_export,
}

fn assert_abi_version(vtable: &'static raw::pokedex_export) {
...
impl VTable {
    pub fn from_dylib(so_path: &str) -> Self {
        ...
        let entry_name = c"EXPORT_ASL_MODEL_get_pokedex_export";

        let entry = unsafe { libc::dlsym(so_lib, entry_name.as_ptr()) };
        let entry: raw::get_pokedex_export_t = unsafe { std::mem::transmute(entry) };

        let entry = entry.unwrap_or_else(|| panic!("dlsym failed: {entry_name:?} not found"));
        let vtable: &'static raw::pokedex_export = unsafe { &*entry() };
    ...
    pub fn bundled() -> Self {
        ...
        unsafe extern "C" {
            #[link_name = "EXPORT_ASL_MODEL_get_pokedex_export"]
...
pub struct ModelHandle {
    data: NonNull<c_void>,
    vtable: &'static raw::pokedex_export,
}
