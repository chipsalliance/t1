...
If a CSR addres not contains in this section get read or write,
...
The `step` function at ASL model will try reset architecture states when the
`FFI_is_reset` function return true. The return value is maintained by the
`is_reset` field in `Simulator`. It can only be accessed by a getter function
shown as follow, which will update the state after read.
