# Modified from https://github.com/riscv/riscv-opcodes/blob/master/src/riscv_opcodes/shared_utils.py
#
# date: 2025-11-08, commit: 26e2c04c913a67ac51bf0a354f21f2d7d5c07c40
#
# Modifications:
#   inline some definitions from constant.py
#   add root_dir parameter
#   disable check_lut_arg
...
# https://github.com/riscv/riscv-opcodes/blob/master/src/riscv_opcodes/constants.py
########################################
##### BEGIN : inlined constants.py #####
########################################
...
def process_enc_line(line: str, ext: str) -> tuple[str, SingleInstr]:
    ...
    # NOTE : we remove this check since we don't have arg_lut constant
...
def update_with_expanded_instructions(
    updated_dict: dict[str, SingleInstr], key: str, value: SingleInstr
):
...
def read_lines(root_dir: Path, file: str) -> list[str]:
    """
...
def process_standard_instructions(
    lines: list[str], instr_dict: dict[str, SingleInstr], file_name: str
):
...
def process_pseudo_instructions(
    ...
    for line in lines:
        ...
        ext_file = read_extension_file(root_dir, ext)
...
def read_extension_file(root_dir: Path, ext: str) -> str:
    """
    Read the extension file path, considering the unratified directory if necessary.
    """
    file = root_dir / "extensions" / ext
    if file.is_file():
        return file.read_text(encoding="utf-8")
    file = root_dir / "extensions" / "unratified" / ext
    if file.is_file():
        return file.read_text(encoding="utf-8")
...
def create_inst_dict(
    ...
    for file_name in file_names:
        ...
        lines = read_lines(root_dir, file_name)
...
def instr_dict_2_extensions(instr_dict: dict[str, SingleInstr]) -> list[str]:
