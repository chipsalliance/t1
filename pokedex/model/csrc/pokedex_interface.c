...
static uint8_t pokedex_step_trace(
    void* _model,
...
const struct pokedex_trace_buffer* pokedex_get_trace_buffer(void* _model) {
    (void)(_model);
...
static void pokedex_read_pc(void* _model, uint64_t* ret){
    (void)(_model);

    *ret = sext(ASL_read_PC_0());
}

static void pokedex_read_xreg(void* _model, uint8_t xs, uint64_t* ret){
    (void)(_model);

    *ret = sext(ASL_read_XREG_0(xs));
}

static void pokedex_read_freg(void* _model, uint8_t fs, uint64_t* ret){
    (void)(_model);

    *ret = nanbox(ASL_read_FREG_0(fs));
}

static void pokedex_read_vreg(void* _model, uint8_t vs, uint8_t* buf, size_t buflen) {
    (void)(_model);
...
static void pokedex_read_csr(void* _model, uint16_t csr, uint64_t* ret){
    (void)(_model);
...
const struct pokedex_model_description* pokedex_get_description(void* _model) {
    (void)(_model);

    return &model_desc;
}

static const struct pokedex_export export = {
    .abi_version = POKEDEX_ABI_VERSION,
    .create = pokedex_create,
    .destroy = pokedex_destroy,

    .get_description = pokedex_get_description,

    .reset = pokedex_reset,
    .step = pokedex_step_trace,
    .step_trace = pokedex_step_trace,
    .get_trace_buffer = pokedex_get_trace_buffer,

    .get_pc = pokedex_read_pc,
    .get_xreg = pokedex_read_xreg,
    .get_freg = pokedex_read_freg,
    .get_vreg = pokedex_read_vreg,
    .get_csr = pokedex_read_csr,
};

POKEDEX_EXPORT const struct pokedex_export* EXPORT_ASL_MODEL_get_pokedex_export() {
  return &export;
}
