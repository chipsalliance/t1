...
// eew(vd) = 2 * sew, eew(vs2) = sew
// compute vd = sext(vs2) - sext(sext(rs1)), optionally masked by vm
...
case sew of
  when 8 => begin
    ...
    let src1 = SInt(src1_)[15:0];

    for idx = 0 to vl - 1 do
      if vm == '0' && V0_MASK[idx] then
        let src2 = VRF_8[vs2, idx];

        let res = SInt(src2)[15:0] - src1;
  ...
  when 16 => begin
    ...
    let src1 = SInt(src1_)[31:0];

    for idx = 0 to vl - 1 do
      if vm =='0' && V0_MASK[idx] then
        let src2 = VRF_16[vs2, idx];

        let res = SInt(src2)[31:0] - src1;
