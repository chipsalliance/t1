...
def get_enabled_extensions() -> list[str]:
    ext_list = []
    for x in Path("template/extensions").iterdir():
        if x.is_dir():
            ext_list.append(x.name)
    return ext_list
...
if __name__ == "__main__":
    ...
    w.variable('RISCV_OPCODES_SRC', os.environ['RISCV_OPCODES_SRC'])
    w.variable('ENABLED_EXTENSIONS', [f'--enable-instruction-sets {ext}' for ext in get_enabled_extensions()])
    w.newline()

    w.rule(
        'gen_build_ninja',
        'python gen_ninja.py > build.ninja',
        description='regenerate build.ninja',
    )
    w.build('build.ninja', 'gen_build_ninja', implicit='gen_ninja.py')
    w.newline()
    ...
    w.build('build/rvopcodes.json', 'gen_rvopcode_instructions')
    w.newline()
    ...
    w.build('build/causes.json', 'gen_rvopcode_causes')
    w.newline()
    ...
    w.rule(
        ...
        description='generate rvopcode csr json',
    )
    w.build('build/csr_meta.json', 'gen_rvopcode_csr', 'template/csr')
    w.newline()

    w.default('build/rvopcodes.json')
    w.default('build/causes.json')
    w.default('build/csr_meta.json')
