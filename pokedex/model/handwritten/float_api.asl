...
func f_is_not_a_number(v : bits(32)) => boolean
begin
  return v[31] == '1' && IsOnes(v[30:21]) && !(IsZero(v[20:0]));
end

func f_is_negative_zero(v : bits(32)) => boolean
begin
...
begin
  case rm of
    ...
    // implementation bug:
    // Dynamic rounding mode should be convert to proper rounding mode store in FRM bit field.
    when RM_DYN => assert FALSE;
  end
end
...
begin
  ...
  let xcpt : integer = ffi_yield_softfloat_exception_flag();
...
begin
  ...
  let is_lt = ffi_f32_lt_quiet(s1, s2) || (ffi_f32_eq(s1, s2) && f_is_negative_zero(s1));
  let xcpt = ffi_yield_softfloat_exception_flag();

  // if both are NaN
  if f_is_not_a_number(s1) && f_is_not_a_number(s2) then
    return FloatOpResult {
      data = CANONICAL_NAN,
      xcpt = xcpt
    };
  ...
  if is_lt || f_is_not_a_number(s2) then
    return FloatOpResult {
      data = s1,
      xcpt = xcpt
    };
...
begin
  ...
  // operand, so that we don't need to return invalid ourselves, keeping
  // consistency with other softfloat operation.
  let is_gt = ffi_f32_gt_quiet(s1, s2) || (ffi_f32_eq(s1, s2) && f_is_negative_zero(s2));
  let xcpt = ffi_yield_softfloat_exception_flag();
  ...
  if is_gt || f_is_not_a_number(s2) then
    return FloatOpResult {
      data = s1,
      xcpt = xcpt
    };
