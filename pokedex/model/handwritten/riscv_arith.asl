...
func riscv_clipSaturateSrl_var{N}(N: integer, x: bits(2*N), y: bits(N), vxrm: bits(2)) => (bits(N), boolean)
begin
...
func riscv_clipSaturateSra_var{N}(N: integer, x: bits(2*N), y: bits(N), vxrm: bits(2)) => (bits(N), boolean)
begin
  var shifted: bits(2*N);
  case N of
