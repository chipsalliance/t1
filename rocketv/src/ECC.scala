...
abstract class Code {
  ...
  /** Takes the unencoded width and returns a list of indices indicating which bits of the encoded value will be used
    * for ecc
    */
  def eccIndices(width: Int): Seq[Int]

  /** Encode x to a codeword suitable for decode. If poison is true, the decoded value will report uncorrectable error
