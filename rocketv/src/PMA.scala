...
class PMAChecker(val parameter: PMACheckerParameter)
  ...
  io.resp.cacheable := legal_address && (if (parameter.cacheable.isEmpty) false.B
                                         else parameter.cacheable.matches(io.paddr))
  ...
  io.resp.eff       := legal_address && (if (parameter.sideEffects.isEmpty) false.B
                                   else parameter.sideEffects.matches(io.paddr))
}
