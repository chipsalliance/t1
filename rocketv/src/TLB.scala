...
class TLB(val parameter: TLBParameter)
  ...
  val mpu_ppn           = Mux[UInt](
    ...
    Mux(
      ...
      special_entry
        .map(e => TLBEntry.ppn(e, vpn, TLBEntry.getData(e, vpn), usingVM, pgLevelBits, true, false))
        .getOrElse(0.U),
  ...
  when(do_refill) {
    ...
    when(special_entry.nonEmpty.B && !io.ptw.resp.bits.homogeneous) {
      special_entry.foreach(tlbEntry =>
        TLBEntry.insert(tlbEntry, r_refill_tag, refill_v, io.ptw.resp.bits.level, newEntry, superpageOnly = false)
      )
  ...
  val priv_rw_ok          =
    Mux(!priv_s || sum, VecInit(entries.map(_.u)).asUInt, 0.U) | Mux(priv_s, ~VecInit(entries.map(_.u)).asUInt, 0.U)
  ...
  val r_array             =
    Cat(
      true.B,
      (priv_rw_ok & (VecInit(entries.map(_.sr)).asUInt | Mux(
        mxr,
        VecInit(entries.map(_.sx)).asUInt,
        0.U
      ))) | stage1_bypass
    )
  ...
  val hr_array            =
    Cat(
      true.B,
      VecInit(entries.map(_.hr)).asUInt | Mux(io.ptw.status.mxr, VecInit(entries.map(_.hx)).asUInt, 0.U) | stage2_bypass
    )
