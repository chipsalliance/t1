...
case class RocketParameter(
  ...
  val mulParameter:            Option[PipelinedMultiplierParameter] = Option.when(usingMulDiv && mulUnroll == xLen)(
...
class Rocket(val parameter: RocketParameter)
  ...
  val fpuDecoder:                       Option[Instance[FPUDecoder]]          =
    Option.when(usingFPU)(Instantiate(new FPUDecoder(parameter.decoderParameter)))
  ...
  class Gated {
    ...
    val exRegBTBResponse:        BTBResp      = Reg(
      new BTBResp(vaddrBits, btbEntries, fetchWidth, bhtHistoryLength, bhtCounterLength)
    )
    ...
    def decodeReg(x: UInt): (Bool, UInt) =
      ((if (x.getWidth - 1 < lgNXRegs) 0.U else x(x.getWidth - 1, lgNXRegs)).asBool, x(lgNXRegs - 1, 0))
    ...
    val idRen:      Seq[Bool] =
      IndexedSeq(idDecodeOutput(parameter.decoderParameter.rxs1), idDecodeOutput(parameter.decoderParameter.rxs2))
    ...
    val idCsrEn:             Bool = isOneOf(
      idDecodeOutput(parameter.decoderParameter.csr),
      Seq(parameter.csrParameter.S, parameter.csrParameter.C, parameter.csrParameter.W)
    )
    val idSystemInstruction: Bool = idDecodeOutput(parameter.decoderParameter.csr) === parameter.csrParameter.I
    val idCsrRen:            Bool = isOneOf(
      idDecodeOutput(parameter.decoderParameter.csr),
      Seq(parameter.csrParameter.S, parameter.csrParameter.C)
    ) && idExpandedInstruction.rs1 === 0.U
    val idCsr      =
      Mux(
        idSystemInstruction && idDecodeOutput(parameter.decoderParameter.mem),
        parameter.csrParameter.N,
        Mux(idCsrRen, parameter.csrParameter.R, idDecodeOutput(parameter.decoderParameter.csr))
      )
    val idCsrFlush =
      idSystemInstruction ||
    ...
    val idDpIllegal:       Option[Bool] =
      Option.when(usingFPU)(idDecodeOutput(parameter.decoderParameter.dp) && !csr.io.status.isa('d' - 'a'))
    ...
    val idFenceNext:  Bool =
      idDecodeOutput(parameter.decoderParameter.fence) || idDecodeOutput(parameter.decoderParameter.amo) && idAmoAquire
    ...
    val bypassSources:    Seq[(Bool, UInt, UInt)] = IndexedSeq(
      ...
      (
        memRegValid && memRegDecodeOutput(parameter.decoderParameter.wxd) && !memRegDecodeOutput(
          parameter.decoderParameter.mem
        ),
        memWaddr,
        wbRegWdata
      ),
    ...
    val exOp1: SInt =
      MuxLookup(exRegDecodeOutput(parameter.decoderParameter.selAlu1), 0.S)(
        Seq(A1_RS1 -> exRs(0).asSInt, A1_PC -> exRegPC.asSInt)
      )
    ...
    mulDiv.io.req.valid    := exRegValid && Option
    ...
    when(!ctrlKilled) {
      ...
      when(
        isOneOf(idDecodeOutput(parameter.decoderParameter.memCommand), Seq(M_SFENCE, M_HFENCEV, M_HFENCEG, M_FLUSH_ALL))
      ) {
...
//    coverExceptions(exException, exCause, "EXECUTE", exCoverCauses)
    ...
    val memBranchTarget:           SInt = memRegPc.asSInt +
      Mux(
        ...
        Mux(
          ...
          ImmGen(ImmGen.IMM_UJ, memRegInstruction),
    ...
    val memCfi:                    Bool =
      memRegDecodeOutput(parameter.decoderParameter.isBranch) || memRegDecodeOutput(
    ...
    val memDirectionMisprediction: Bool =
      memRegDecodeOutput(
        parameter.decoderParameter.isBranch
      ) && memBranchTaken =/= (usingBTB.B && memRegBTBResponse.taken)
    ...
    when(memRegValid && memRegFlushPipe) {
      ...
      memRegLoad        := exRegDecodeOutput(parameter.decoderParameter.mem) && isRead(
        exRegDecodeOutput(parameter.decoderParameter.memCommand)
      )
      memRegStore       := exRegDecodeOutput(parameter.decoderParameter.mem) && isWrite(
        exRegDecodeOutput(parameter.decoderParameter.memCommand)
      )
      ...
      when(
        ...
        exRegDecodeOutput(parameter.decoderParameter.rxs2) && Option
          .when(usingVector)(exRegDecodeOutput(parameter.decoderParameter.vector))
          .getOrElse(false.B)
      ) {
        // for setvl
        memRegRS2 := exRs(1)
...
//    coverExceptions(memException, memCause, "MEMORY", memCoverCauses)
    ...
    val dcacheKillMem =
      memRegValid && memRegDecodeOutput(
        parameter.decoderParameter.wxd
      ) && io.dmem.replay_next // structural hazard on writeback port
    // TODO: vectorKillMem?
    ...
    when(memPcValid) {
      ...
      wbRegWdata          := io.fpu
        .map(fpu =>
          Mux(
            !memRegException && memRegDecodeOutput(parameter.decoderParameter.fp) && memRegDecodeOutput(
              parameter.decoderParameter.wxd
            ),
            fpu.toint_data,
            Mux(
              !memRegException && Option
                .when(usingVector)(memRegDecodeOutput(parameter.decoderParameter.vectorReadFRs1))
                .getOrElse(false.B),
              fpu.store_data,
              memIntWdata
            )
      ...
      when(
        memRegSfence || Option
          .when(usingVector)(memRegDecodeOutput(parameter.decoderParameter.vector))
          .getOrElse(false.B)
      ) {
        wbRegRS2 := memRegRS2
      }
      ...
      wbRegWphit.lazyZip(memRegWphit).lazyZip(breakpointUnit.io.bpwatch).foreach {
        case (wbRegWphit, memRegWphit, bpw) =>
    ...
    val (wbException, wbCause) = checkExceptions(
      List(
        ...
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.pf.st,
          Causes.store_page_fault.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.pf.ld,
          Causes.load_page_fault.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.gf.st,
          Causes.store_guest_page_fault.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.gf.ld,
          Causes.load_guest_page_fault.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.ae.st,
          Causes.store_access.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.ae.ld,
          Causes.load_access.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.ma.st,
          Causes.misaligned_store.U
        ),
        (
          wbRegValid && wbRegDecodeOutput(parameter.decoderParameter.mem) && io.dmem.s2_xcpt.ma.ld,
      ...
      wbDcacheMiss ||
        ...
        Option
          .when(usingVector) {
            ...
            wbRegDecodeOutput(parameter.decoderParameter.wxd) && wbRegDecodeOutput(
              parameter.decoderParameter.vector
...
//    io.ptw.customCSRs.csrs.zip(csr.io.customCSRs).foreach { case (lhs, rhs) => lhs <> rhs }
    ...
    val exCannotBypass: Bool         =
      exRegDecodeOutput(parameter.decoderParameter.csr) =/= parameter.csrParameter.N ||
        ...
        exRegDecodeOutput(parameter.decoderParameter.mem) ||
        Option
          .when(usingMulDiv && pipelinedMul)(exRegDecodeOutput(parameter.decoderParameter.mul))
          .getOrElse(false.B) ||
    ...
    val memCannotBypass: Bool         =
      memRegDecodeOutput(parameter.decoderParameter.csr) =/= parameter.csrParameter.N ||
        ...
        Option
          .when(usingMulDiv && pipelinedMul)(memRegDecodeOutput(parameter.decoderParameter.mul))
          .getOrElse(false.B) ||
    ...
    val idStallFpu:     Bool =
      io.fpu
        .zip(fpHazardTargets)
        .map { case (fpu, fpHazardTargets) =>
          val fpScoreboard = new Scoreboard(32)
          // 8. set FP scoreboard
          fpScoreboard.set(
            ((wbDcacheMiss || Option
              .when(usingVector)(wbRegDecodeOutput(parameter.decoderParameter.vector))
              .getOrElse(false.B)) && wbRegDecodeOutput(parameter.decoderParameter.wfd) || fpu.sboard_set) && wbValid,
            wbWaddr
          )
    ...
    val vectorStall:     Option[Bool] = Option.when(usingVector) {
      ...
      (idDecodeOutput(parameter.decoderParameter.mem) && !idDecodeOutput(
        parameter.decoderParameter.vector
      ) && vectorLSUNotClear)
    }
    ...
    io.imem.btb_update.bits.cfiType    :=
      Mux(
        (memRegDecodeOutput(parameter.decoderParameter.isJal) || memRegDecodeOutput(
          parameter.decoderParameter.isJalr
        )) && memWaddr(0),
        CFIType.call,
        Mux(
          memRegDecodeOutput(parameter.decoderParameter.isJalr) && (memRegInstruction(
            19,
            15
          ) & regAddrMask.U) === BitPat("b00?01"),
          CFIType.ret,
          Mux(
            memRegDecodeOutput(parameter.decoderParameter.isJal) || memRegDecodeOutput(
              parameter.decoderParameter.isJalr
...
//      fpu.keep_clock_enabled := io.ptw.customCSRs.disableCoreClockGate
    ...
    val ex_dcache_tag =
      Cat(exWaddr, Option.when(usingFPU)(exRegDecodeOutput(parameter.decoderParameter.fp)).getOrElse(false.B))
//    require(coreParams.dcacheReqTagBits >= ex_dcache_tag.getWidth)
    ...
    io.dmem.s1_data.data      := io.fpu
      .map(fpu =>
        Mux(
          memRegDecodeOutput(parameter.decoderParameter.fp),
          Fill(xLen.max(fLen.get) / fLen.get, fpu.store_data),
          memRegRS2
        )
      )
      .getOrElse(memRegRS2)
    ...
    io.dmem.keep_clock_enabled := instructionBufferOut.valid && idDecodeOutput(
      parameter.decoderParameter.mem
    ) && !csr.io.csrStall
