...
case class HellaCacheParameter(
  ...
  def b2i(i: Boolean): Int = if (i) 1 else 0
  ...
  val scratch: Option[BigInt] = None
  val separateUncachedResp: Boolean = false
  val tagECC: Option[String] = None
  val dataECC: Option[String] = None
  val nTLBSets: Int = 1
  val nTLBWays: Int = 32
  val nTLBBasePageSectors: Int = 4
  val nTLBSuperpages: Int = 4
  val useAsyncReset: Boolean = true

  // todo: param or from bus?
  val allAddressRange: BitPat = BitPat.dontCare(paddrBits)
  val allAddressBitPat: BitSet = BitSet(allAddressRange)
  val emptyAddressBitPat: BitSet = BitSet()
  val  legal:       BitSet = allAddressBitPat
  val  cacheable:   BitSet = allAddressBitPat
  val  read:        BitSet = allAddressBitPat
  val  write:       BitSet = allAddressBitPat
  val  putPartial:  BitSet = allAddressBitPat
  val  logic:       BitSet = emptyAddressBitPat
  val  arithmetic:  BitSet = emptyAddressBitPat
  val  exec:        BitSet = allAddressBitPat
  val  sideEffects: BitSet = allAddressBitPat

  val lgCacheBlockBytes: Int = log2Ceil(cacheBlockBytes)
  val blockOffBits: Int = lgCacheBlockBytes
  ...
  val usingDataScratchpad: Boolean = scratch.isDefined
  val dcacheArbPorts: Int = 1 + b2i(usingVM) + b2i(usingDataScratchpad)
  val tagCode: Code = Code.fromString(tagECC)
  val dataCode: Code = Code.fromString(dataECC)

  val pgLevelBits:            Int = 10 - log2Ceil(xLen / 32)
  val pipelineWayMux: Boolean = false
  val nPMPs:                Int = 8
  val vpnBits:              Int = vaddrBits - pgIdxBits
  val hasCorrectable:       Boolean = tagCode.canCorrect || dataCode.canCorrect
  val hasUncorrectable:     Boolean = tagCode.canDetect || dataCode.canDetect
  ...
  val acquireBeforeRelease: Boolean = false
  val silentDrop: Boolean = !acquireBeforeRelease
  val clockGate: Boolean = false
  val replacementPolicy: String = "random" //lfsr
  val idxMSB: Int = untagBits - 1
  val idxLSB: Int = blockOffBits
  val subWordBits: Option[Int] = None
  val wordBits: Int = coreDataBits
  val rowWords: Int = rowBits / wordBits
  val wordBytes: Int = coreDataBytes
  val wordOffBits: Int = log2Ceil(wordBytes)
  val cacheDataBits: Int = rowBits
  val cacheDataBeats: Int = (cacheBlockBytes * 8) / cacheDataBits
  val beatBytes: Int = cacheBlockBytes / cacheDataBeats
  val beatWords: Int = beatBytes / wordBytes
  val dataECCBytes: Int = 1
  val eccBits: Int = dataECCBytes * 8
  val eccBytes: Int = dataECCBytes
  val encBits: Int = dataCode.width(eccBits)
  val rowBytes: Int = rowBits / 8
  val subWordBytes: Int = subWordBits.getOrElse(wordBits) / 8
  val rowOffBits: Int = log2Ceil(rowBytes)
  val beatOffBits: Int = log2Ceil(beatBytes)

  val usingAtomics: Boolean = true
  val useAtomicsOnlyForIO: Boolean = false
  val usingAtomicsInCache: Boolean = usingAtomics && !useAtomicsOnlyForIO

  val maxUncachedInFlight: Int = nMMIOs
  val pgUntagBits: Int = if (usingVM) untagBits min pgIdxBits else untagBits
  //            tlBundleParams.addressBits - pgUntagBits
  val tagBits: Int = paddrBits - pgUntagBits

  // todo: max axi id
  val firstMMIO: Int = 4
  val lrscBackoff: Int = 3
  ...
  val pmaCheckerParameter: PMACheckerParameter = PMACheckerParameter(
    paddrBits,
    legal,
    cacheable,
    read,
    ...
    exec,
    sideEffects)
  val tlbParameter:        TLBParameter = TLBParameter(
    useAsyncReset,
    xLen,
    nTLBSets,
    nTLBWays,
  ...
  val arbiterParameter:    HellaCacheArbiterParameter = HellaCacheArbiterParameter(
    dcacheArbPorts,
    coreMaxAddrBits,
    usingVM,
    untagBits,
  ...
  val amoaluParameter:     AMOALUParameter = AMOALUParameter(xLen)
  val dtimParameter: Option[AXI4BundleParameter] = scratch.map { _ =>
    AXI4BundleParameter(
      idWidth = 1,
      dataWidth = rowBits,
      addrWidth = paddrBits,
    ...
    val s2_data = {
      ...
      if (cacheParams.pipelineWayMux) {
        ...
        val s1_word_en = Mux(
          !io.cpu.replay_next,
          word_en,
          UIntToOH(uncachedResp.addr(log2Ceil(rowBits / 8) - 1, log2Ceil(wordBytes)), wordsPerRow)
        )
        VecInit(for (i <- 0 until wordsPerRow) yield {
...
//    val s2_hit_valid = s2_hit_state.isValid()
    ...
    dataArb.io.in(0).bits.wordMask := {
      ...
      val wordMask = UIntToOH(Mux(pstore2_valid, pstore2_addr, pstore1_addr)(rowOffBits - 1, log2Ceil(wordBytes)))
...
//    metaArb.io.in(3).valid := grantIsCached && d_done && !tl_out.d.bits.denied
    ...
    // AMOs
    if (usingRMW) {
      val amoalus = (0 until parameter.coreDataBits / parameter.xLen).map { i =>
        val amoalu = Module(new AMOALU(parameter.amoaluParameter))
