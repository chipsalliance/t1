...
class PTW(val parameter: PTWParameter)
  ...
  class PMPHomogeneityChecker(pmps: Seq[PMP]) {
    def apply(addr: UInt, pgLevel: UInt): Bool = {
      pmps
        .foldLeft((true.B, 0.U.asTypeOf(chiselTypeOf(pmps.head)))) { case ((h, prev), pmp) =>
          (
            h && PMP.homogeneous(pmp, addr, pgLevel, prev, paddrBits, pmpGranularity, pgLevels, pgIdxBits, pgLevelBits),
            pmp
          )
        }
        ._1
      ...
      if (coreParams.nL2TLBEntries == 0) (false.B, false.B, WireDefault(0.U.asTypeOf(new PTE)), None)
        ...
        val ram: SRAMInterface[Vec[UInt]] = SRAM.masked(
          ...
          tpe = Vec(
            ...
            UInt(code.width(new L2TLBEntry(nL2TLBSets, ppnBits, maxSVAddrBits, pgIdxBits, usingHypervisor).getWidth).W)
          ),
          numReadPorts = 0,
          numWritePorts = 0,
          numReadwritePorts = 1
        ...
        val s2_error     =
          VecInit((0 until coreParams.nL2TLBWays).map(way => s2_valid_vec(way) && s2_rdata(way).error)).asUInt.orR
        ...
        // decode
        val s2_entry_vec = s2_rdata.map(
          _.uncorrected.asTypeOf(new L2TLBEntry(nL2TLBSets, ppnBits, maxSVAddrBits, pgIdxBits, usingHypervisor))
        )
        val s2_hit_vec   =
