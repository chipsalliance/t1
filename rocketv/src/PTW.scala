...
class PTW(val parameter: PTWParameter)
  ...
  withClock(gated_clock) { // entering gated-clock domain
    ...
    io.mem.uncached_resp.map { resp =>
      AssertProperty(BoolSequence(!(resp.valid && io.mem.resp.valid)), clock = Some(gated_clock))
      resp.ready := true.B
      when(resp.valid) {
    ...
    val (l2_hit, l2_error, l2_pte, l2_tlb_ram) =
      if (coreParams.nL2TLBEntries == 0) (false.B, false.B, WireDefault(0.U.asTypeOf(new PTE)), None)
        ...
        when(s2_hit) {
          ...
          AssertProperty(BoolSequence((PopCount(s2_hit_vec) === 1.U) || s2_error), clock = Some(gated_clock))
        }
...
//      io.requestor(i).customCSRs <> io.dpath.customCSRs
    ...
    switch(state) {
      is(s_ready) {
        when(arb.io.out.fire) {
          ...
          AssertProperty(BoolSequence(!arb.io.out.bits.bits.need_gpa || arb.io.out.bits.bits.stage2), clock = Some(gated_clock))
        }
      }
      is(s_req) {
    ...
    when(l2_hit && !l2_error) {
      AssertProperty(BoolSequence(state === s_req || state === s_wait1), clock = Some(gated_clock))
      next_state := s_ready
      resp_valid(r_req_dest) := true.B
      count := (pgLevels - 1).U
    }
    when(mem_resp_valid) {
      AssertProperty(BoolSequence(state === s_wait3), clock = Some(gated_clock))
      next_state := s_req
      when(traverse) {
    ...
    when(io.mem.s2_nack) {
      AssertProperty(BoolSequence(state === s_wait2), clock = Some(gated_clock))
      next_state := s_req
    }
