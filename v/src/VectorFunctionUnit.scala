...
class SlotExecuteRequest(slotIndex: Int, parameter: VFUInstantiateParameter) extends Record with AutoCloneType {
  val elements: SeqMap[String, Data] = SeqMap.from(
    parameter.genVec.filter(_._2.contains(slotIndex)).map {
      case (p, _) =>
...
object vfu {
  def vfuConnect(
    ...
    requestVecFromSlot.zipWithIndex.foreach {
      case (request, slotIndex) =>
        val requestFire = request.elements.map {
          case (name: String, reqForVfu: DecoupledIO[Bundle]) =>
            // 检测类型
            ...
            // 连接bits
            reqForVfu.bits.elements.foreach {
              case (s, d) =>
                d match {
                  // src 不等长,所以要特别连
                  case src: Vec[Data] =>
                    src.zipWithIndex.foreach { case (sd, si) => sd := requestVec(slotIndex).src(si) }
    ...
    // 处理vfu
    val vfuResponse: Seq[ValidIO[VFUResponseToSlot]] = parameter.genVec.zipWithIndex.map {
      case ((gen, slotVec), vfuIndex) =>
        // vfu 模块
        ...
        requestArbiter.io.in.zip(slotVec).foreach {
          case (arbiterInput, slotIndex) =>
        ...
        responseBundle.bits.elements.foreach {
          case (name, data) =>
    ...
    // 把response丢给slot
    responseVec.zipWithIndex.foreach {
      case (data, slotIndex) =>
        // 筛选 response
        val responseFilter: Seq[(Bool, ValidIO[VFUResponseToSlot])] =
          vfuResponse.zip(parameter.genVec).filter(_._2._2.contains(slotIndex)).map {
            case (resp, (gen, _)) =>
