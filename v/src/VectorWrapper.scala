...
class VectorWrapper(parameter: VParameter) extends Module {
  ...
  // 先忽视v set 类型的指令
  val vSetInstruction: Bool =
    (request.bits.instruction(6, 0) === "0b1010111".U) && (request.bits.instruction(14, 12) === 7.U)
  val instructionQueue: Queue[InstructionQueueBundle] = Module(
    new Queue(new InstructionQueueBundle(parameter), parameter.instructionQueueSize)
  )
