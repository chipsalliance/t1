...
class SRTWrapper extends Module {
  ...
  srt.input.bits.dividend := Mux(
    ...
    abs.io.aOut >> -leftShiftWidthDividend,
    abs.io.aOut << leftShiftWidthDividend
  )
  ...
  val remainderBias = Wire(UInt(32.W))
  val bias = Wire(UInt(1.W))
  quotientAbs := srt.output.bits.quotient
  remainderBias := srt.output.bits.reminder >> zeroHeadDivisorSRT
  bias := Mux(leftShiftWidthDividend(5), Mux(dividend(0), 1.U, 0.U), 0.U)
  remainderAbs := bias + remainderBias
  ...
  output.bits.reminder := Mux(
    divideZeroReg,
    dividendRestore,
    Mux(biggerdivisorReg, dividendRestore, Mux(dividendSignSRT, -remainderAbs, remainderAbs))
  ).asSInt
}
